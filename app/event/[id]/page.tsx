      ...
      try {
        ...
        if (res.ok) {
          ...
          if (currentEvent) {
            ...
            // Thiết lập phòng hờ nếu DB trống
            setEventData({
              id: params.id,
              name: "Giải Thể Thao Thử Thách",
              creator_name: "Ban Tổ Chức MyRace",
  ...
  const handleJoinEvent = async () => {
    ...
    try {
      ...
      if (res.ok || result.message?.includes("thành công")) {
        alert(`🎉 XIN CHÚC MỪNG! Sếp đã ghi danh vào giải đấu "${eventData.name}" thành công! Hệ thống Trọng tài AI đã lập phòng thi đấu cho sếp.`);
      } else {
          ...
          <div className="grid md:grid-cols-2 gap-6">
            ...
            {eventData.sports_included?.run && eventData.rules?.run && (
              <div className="border-2 border-orange-100 rounded-2xl p-5 bg-orange-50/10">
                ...
                <ul className="space-y-3 text-sm font-medium text-gray-600">
                  <li className="flex justify-between border-b pb-2"><span>Khung Pace hợp lệ:</span> <strong className="text-gray-900">{eventData.rules.run.paceMin} - {eventData.rules.run.paceMax} min/km [cite: 11]</strong></li>
                  <li className="flex justify-between border-b pb-2"><span>Hiệu suất vận động:</span> <strong className="text-gray-900">{">="} {eventData.rules.run.movingRatio}% (Elapsed Time) [cite: 2, 5]</strong></li>
            ...
            {eventData.sports_included?.ride && eventData.rules?.ride && (
              <div className="border-2 border-blue-100 rounded-2xl p-5 bg-blue-50/10">
                ...
                <ul className="space-y-3 text-sm font-medium text-gray-600">
                  <li className="flex justify-between border-b pb-2"><span>Tốc độ hợp lệ:</span> <strong className="text-gray-900">{eventData.rules.ride.speedMin} - {eventData.rules.ride.speedMax} km/h [cite: 12]</strong></li>
                  <li className="flex justify-between border-b pb-2"><span>Hiệu suất vận động:</span> <strong className="text-gray-900">{">="} {eventData.rules.ride.movingRatio}% (Elapsed Time) [cite: 2, 6]</strong></li>
                  <li className="flex justify-between border-b pb-2"><span>Cự ly tối thiểu/cuốc:</span> <strong className="text-gray-900">{eventData.rules.ride.minDistance} km [cite: 14]</strong></li>
                  {eventData.rules.pointsConversion && (
